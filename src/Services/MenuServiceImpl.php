...
class MenuServiceImpl
{
    ...
    public function create(array $data)
    {
        return DB::transaction(function () use ($data) {
            ...
            // 处理数据
            $data->path = $data->pid == 0 ? $data->id : $data->pid . ',' . $data->id;
    ...
    public function findParentByMenus(SupportCollection $menus)
    {
        $path = $menus->pluck('path');
        $ids = $path->map(function ($item) {
            return explode(',', $item);
